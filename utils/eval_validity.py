...
from utils.eval_bond_mmd import *

# --- Updated atom definitions for QM9 and GEOM-Drugs ---
atomic_valence = defaultdict(list)
atomic_valence[1] = [1]      # H
...
atomic_valence[53] = [1]     # I


# Covalent radii in Angstrom for bond length threshold calculation
COVALENT_RADII = {
    1: 0.31, 6: 0.76, 7: 0.71, 8: 0.66, 9: 0.57,
    15: 1.07, 16: 1.05, 17: 1.02, 35: 1.20, 53: 1.39
}

# --- Generalized bond thresholds based on covalent radii ---
# This approach is more robust and extensible than the previous hardcoded values.
atomic_valid_bond = defaultdict(float)
# The 1.3 factor is a tolerance, as suggested in the original docstring.
COVALENT_FACTOR = 1.3
for atom1 in COVALENT_RADII:
    for atom2 in COVALENT_RADII:
        # We only need to store one entry for each pair (e.g., (6, 7) but not (7, 6))
        if atom1 <= atom2:
            radius_sum = COVALENT_RADII[atom1] + COVALENT_RADII[atom2]
            atomic_valid_bond[(atom1, atom2)] = radius_sum * COVALENT_FACTOR
...
def calculate_metric(mol_dicts, dataset_name='qm9', debug = False):
    ...
    # Determine which SMILES file to use for the novelty check
    if dataset_name == 'qm9':
        smiles_file = 'data/qm9_smiles_list.txt'
    elif dataset_name == 'geom_drugs':
        smiles_file = 'data/geom_drugs_smiles_10_36.txt'
    else:
        raise ValueError(f"Unknown dataset for novelty check: '{dataset_name}'")

    if not os.path.exists(smiles_file):
        print(f"Warning: SMILES file for novelty check not found at '{smiles_file}'. Skipping novelty calculation.")
        novel_ratio = -1.0 # Return a specific value indicating novelty could not be calculated
    else:
        # Use a set for efficient lookup
        train_smiles = set(line.strip() for line in open(smiles_file, 'r'))
        ...
        novel_ratio = novel_number / n_valid if n_valid > 0 else 0
    
    if debug:
        return n_valid/n_generated, unique_ratio, novel_ratio, 0, valid_list, con_mat_list
    print(f"calculating bond mmd...")
